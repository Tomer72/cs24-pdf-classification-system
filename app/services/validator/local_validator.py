...
class ExamValidator(BaseValidator):
    def validate(self, extracted_text: str, metadata: Dict[str, Any]) -> bool:
        ...
        if not self.is_test_context(extracted_text):
            logger.warning("Validation failed: Document does not appear to be an exam.")
            return False

        match_count = self.count_field_matches(extracted_text, metadata)
